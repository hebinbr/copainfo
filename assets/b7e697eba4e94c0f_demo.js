...
$(function () {
  var popupId = "#popup-subscribe";
  var storageKey = "subscribePopupShown";
  var delayMs = 20000;
  ...
  setTimeout(function () {
    ...
    if ($(".js-popup.active").length) return;
...
$(function () {
  function normalizeMailchimpSubmit($form) {
    $form.find(".clear .button.button__secondary").each(function () {
      var $legacy = $(this);
      if ($legacy.is("button")) return;

      var $submit = $legacy.find('input[type="submit"]').first();
      if (!$submit.length) return;

      var $button = $("<button>", {
        type: "submit",
        class: $legacy.attr("class") || "button button__secondary",
      });
      var $label = $("<span>", {
        class: "button-text",
        text: $submit.val() || "Subscribe",
      });

      if ($submit.attr("name")) {
        $button.attr("name", $submit.attr("name"));
      }
      if ($submit.attr("value")) {
        $button.attr("value", $submit.attr("value"));
      }
      if ($submit.attr("id")) {
        $button.attr("id", $submit.attr("id"));
      }
  ...
  function bindMailchimpForm($form) {
    if ($form.data("mc-bound")) return;
    $form.data("mc-bound", true);
    normalizeMailchimpSubmit($form);

    function showSuccess($shell, $form, $message) {
      if ($shell.length) {
        $shell.hide();
      } else {
        $form.hide();
    ...
    $form.on("submit", function (e) {
      ...
      var $container = $form.closest(
        ".footer-form-wrapper, .popup-modal-content, .popup-modal",
      );
      var $message = $container.find(".footer-form-cell__message").first();
      var $shell = $container.find("#mc_embed_shell").first();
      var $error = $form.find("#mce-error-response").first();
      var $success = $form.find("#mce-success-response").first();
      var submitSelector = 'button[type="submit"], input[type="submit"]';
      var $submit = $form.find(submitSelector).first();
      var fields = {};

      if ($error.length) $error.hide().text("");
      if ($success.length) $success.hide().text("");
      ...
      $.ajax({
        url:
          window.fifaMailchimpProxy && window.fifaMailchimpProxy.ajaxUrl
            ? window.fifaMailchimpProxy.ajaxUrl
            : "/wp-admin/admin-ajax.php",
        method: "POST",
        dataType: "json",
        timeout: 20000,
        data: {
          action: "fifa_mailchimp_subscribe",
          nonce:
            window.fifaMailchimpProxy && window.fifaMailchimpProxy.nonce
              ? window.fifaMailchimpProxy.nonce
              : "",
          action_url: action,
          fields: fields,
        },
        success: function (resp) {
          if (resp && resp.success) {
            showSuccess($shell, $form, $message);
          } else {
            var msg =
        ...
        error: function (_jqXHR) {
          if ($error.length) {
            $error.text("Subscription failed. Please try again later.").show();
          }
        },
        complete: function () {
  ...
  $(".footer-form-cell__message").hide();

  $('form[action*="list-manage.com/subscribe/post"]').each(function () {
    bindMailchimpForm($(this));
  });
});
...
const appVars = () => {
  ...
  document.documentElement.style.setProperty(
    "--app-width",
    $("body").prop("clientWidth") + "px",
  );
  document.documentElement.style.setProperty(
    "--headroom-height",
    $("#headroom").innerHeight() + "px",
  );
...
$(function () {
  ...
  new ScrollMagic.Scene({
    ...
    .setClassToggle($(".header")[0], "hide-header")
    .addTo(controller);
  ...
  /*
    $(".s20 .carousel").each(function () {
      ...
      const swiperOptions = {
        ...
        on: {
          init(sw) {
            if (sw.params.autoplay && sw.autoplay && !sw.autoplay.running) {
              sw.autoplay.start();
            }
          },
        },
  ...
  $(".s21").each(function (i) {
    ...
    $(this)
      .find(".s21-3 .carousel-button__prev")
      .click(function (e) {
        e.preventDefault();
        mySwiper.slidePrev();
      });
    ...
    $(this)
      .find(".s21-3 .carousel-button__next")
      .click(function (e) {
        e.preventDefault();
        mySwiper.slideNext();
      });
  ...
  $(".page-pattern").each(function (i) {
    new ScrollMagic.Scene({
      ...
      .setTween(
        new TimelineMax().to($(".page-pattern"), 1.0, {
  ...
  $(
    ".footer-nav a, .topbar-nav > ul > li > a, .button__secondary .button-text, .button__clear .button-text, .bottombar a, .scrl-button, .tab-inner",
  ).each(function (i) {
    var box = $(this).html(),
      txt = "<span>" + box + "</span>",
      inner = $("<span>", {
